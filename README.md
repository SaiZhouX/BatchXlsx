...
3. **批量文件处理**
   - 方式1：点击"添加文件夹"按钮，选择包含多个Excel文件的文件夹
   - 方式2：多次点击"添加单个Excel文件"按钮，选择多个文件
   - 系统会自动识别文件数量，超过1个文件时自动使用批量处理模式
   - 点击"开始分析"执行批量处理
...
### 自动化打包工具 (build_exe.py)

项目提供了自动化打包脚本 `build_exe.py`，可以一键将Python项目打包成独立的exe文件。
...
3. **打包参数**
   - `--onefile`: 打包成单个exe文件
   - `--windowed`: 无控制台窗口模式
   - `--name=BatchXlsxTool`: 自定义exe文件名
   - `--clean`: 自动清理临时文件

4. **智能检测**
   - 自动检测是否存在图标文件 (`icon.ico`)
   - 自动检测是否存在配置文件夹 (`config/`)
   - 如果不存在，自动移除相关参数
...
# 方法2: 直接使用PyInstaller
python -m PyInstaller --onefile --windowed --name="BatchXlsxTool" --clean main.py
```

#### 生成文件

打包完成后会在 `dist/` 文件夹中生成：
- `BatchXlsxTool.exe` - 独立可执行文件 (约110MB)
...
- `output/` - 存放生成的分析报告
- `logs/` - 存放程序运行日志
- `dist/` - 存放打包生成的exe文件（仅在打包时创建）

注意：不再需要手动创建input文件夹，所有文件选择都通过GUI界面完成。
